...
type FilterWarehouse = "All" | "Warehouse A" | "Warehouse B";
...
export default function Home() {
    ...
    // Filter & Sort
    const [filterWarehouse, setFilterWarehouse] = useState<FilterWarehouse>("All");
        ...
        try {
            ...
            if (data.length > 0) {
                setNewProduct((prev) => ({
                    ...
                    warehouse: data[0].name,
                }));
            }
        } catch {
            // keep empty warehouse list on failure
        }
    }
        ...
        <div style={{ display: "flex", minHeight: "100vh", background: "#F5F4F0", fontFamily: "'DM Sans', Arial, sans-serif" }}>
            ...
            <aside style={{
                ...
                {[
                    ...
                    <button key={label} onClick={() => { setActiveTab(label); if (label === "Settings") setSettingsDraft({ ...settings }); }}
                        ...
                        {label === "Reservations" && confirmedRes > 0 && (
                                ...
                                return (
                                    <div key={p.inventoryId} style={{ marginBottom: "12px" }}>
                                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", marginBottom: "4px" }}>
                                            ...
                                            <span style={{ color: "#8A8880" }}>{p.availableStock} units</span>
                                        </div>
                            ...
                            <div style={{ ...S.card, padding: "20px" }}>
                                ...
                                {reservations.length === 0
                                    ...
                                    : reservations.slice(0, 4).map((r) => (
                                        <div key={r.id} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #F5F4F0", fontSize: "12.5px" }}>
                                            <div>
                                                <div style={{ fontWeight: 500, color: "#1A1916" }}>{r.product}</div>
                                                <div style={{ color: "#8A8880" }}>{r.quantity} unit{r.quantity > 1 ? "s" : ""}</div>
                                            </div>
                                            <span style={{ fontSize: "11px", fontWeight: 600, padding: "2px 8px", borderRadius: "20px", alignSelf: "center", background: r.status === "confirmed" ? "#F0FDF4" : "#FEF2F2", color: r.status === "confirmed" ? "#15803D" : "#DC2626" }}>
                                                {r.status}
                                            </span>
                                        </div>
                                    ))}
                ...
                {activeTab === "Inventory" && (
                    <div>
                        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "32px" }}>
                            ...
                            <button style={S.primaryBtn} onClick={() => setShowAddProduct(true)}>＋ Add product</button>
                        </div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "14px", marginBottom: "28px" }}>
                            {[
                                ...
                                { value: 2, label: "Warehouses", delta: "All operational", up: true },
                        ...
                        <div style={{ display: "flex", gap: "10px", marginBottom: "20px", flexWrap: "wrap" }}>
                            ...
                            <Dropdown label="Warehouse" options={["All", "Warehouse A", "Warehouse B"] as FilterWarehouse[]} value={filterWarehouse} onChange={setFilterWarehouse} />
                            ...
                            {loading ? (
                                ...
                                return (
                                    <div key={item.inventoryId} style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1fr 1fr 160px", padding: "14px 20px", borderBottom: index < filtered.length - 1 ? "1px solid #E8E6E0" : "none", alignItems: "center" }}>
                                        <div>
                                            <div style={{ fontSize: "14px", fontWeight: 600, color: "#1A1916" }}>{item.product}</div>
                                            <div style={{ fontSize: "12px", color: "#8A8880", marginTop: "1px" }}>#PRD-{String(item.productId).padStart(4, "0")}</div>
                                        </div>
                                        <div>
                ...
                {activeTab === "Reservations" && (
                    <div>
                        ...
                        {reservations.length === 0 ? (
                            ...
                            <div style={{ ...S.card, overflow: "hidden" }}>
                                <div style={{ display: "grid", gridTemplateColumns: "2fr 1.2fr 0.5fr 1.5fr 1fr 80px", padding: "12px 20px", background: "#F5F4F0", borderBottom: "1px solid #E8E6E0" }}>
                                    {["Product", "Warehouse", "Qty", "Reserved at", "Status", "Action"].map((h) => (
                                        <span key={h} style={S.label}>{h}</span>
                                    ))}
                                </div>
                                {reservations.map((r, i) => (
                                    <div key={r.id} style={{ display: "grid", gridTemplateColumns: "2fr 1.2fr 0.5fr 1.5fr 1fr 80px", padding: "14px 20px", borderBottom: i < reservations.length - 1 ? "1px solid #E8E6E0" : "none", alignItems: "center" }}>
                                        <div>
                                            <div style={{ fontSize: "14px", fontWeight: 600, color: "#1A1916" }}>{r.product}</div>
                                            <div style={{ fontSize: "12px", color: "#8A8880" }}>#RES-{String(r.id).slice(-4)}</div>
                                        </div>
                                        ...
                                        {r.status === "confirmed" ? (
                                            <button onClick={() => handleCancelReservation(r.id)} style={{ padding: "6px 10px", border: "1px solid #FEE2E2", borderRadius: "7px", fontSize: "11.5px", fontWeight: 600, background: "#FEF2F2", color: "#DC2626", cursor: "pointer", fontFamily: "inherit" }}>
                                                Cancel
                                            </button>
                                        ) : <div />}
                                    </div>
                                            ...
                                            {products.map((p, i) => {
                                                ...
                                                return (
                                                    <tr key={p.inventoryId} style={{ borderBottom: "1px solid #F5F4F0", background: i % 2 === 0 ? "#fff" : "#FAFAF8" }}>
                                                        <td style={{ padding: "10px 14px", fontWeight: 500, color: "#1A1916" }}>{p.product}</td>
                            ...
                            <div style={{ ...S.card, padding: "24px", gridColumn: "1 / -1" }}>
                                ...
                                <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
                                    ...
                                    <button
                                        ...
                                        onClick={() => { if (confirm("Clear all reservations?")) { setReservations([]); showToast("Reservations cleared"); } }}
                                    >
                                        🗑 Clear reservations
                                    </button>
                                </div>
            ...
            {showAddProduct && (
                <Modal title="Add new product" onClose={() => setShowAddProduct(false)}>
                    ...
                    <div style={{ marginBottom: "14px" }}>
                        ...
                        <select style={S.input} value={newProduct.warehouseId}
                            onChange={(e) => {
                    ...
                    <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px" }}>
                        ...
                        <button style={S.primaryBtn} onClick={handleAddProduct}>Add product</button>
                    </div>
                </Modal>
            )}
            ...
            {editProduct && (
                <Modal title="Edit product" onClose={() => setEditProduct(null)}>
                    <div style={{ marginBottom: "14px" }}>
                        <div style={{ ...S.label, marginBottom: "6px" }}>Product name</div>
                        <input style={S.input} value={editProduct.product} onChange={(e) => setEditProduct({ ...editProduct, product: e.target.value })} />
                    </div>
                    <div style={{ marginBottom: "14px" }}>
                        <div style={{ ...S.label, marginBottom: "6px" }}>Warehouse</div>
                        <select style={S.input} value={editProduct.warehouseId}
                            onChange={(e) => {
                                const wh = warehouses.find((w) => w.id === e.target.value);
